...
import { interfacePins, interfacePanelWidth, IFACE_HEIGHT, IFACE_PAD, IFACE_PIN_SPACING, resolveInterfacePinPos } from '@/utils/surfaceGeometry'
...
/** Festes Übergabefeld – auf Innenausbau und Außeneinheit gespiegelt. */
export default function InterfacePanelBlock({ panel, surface }: Props) {
  ...
  const w = interfacePanelWidth(panel.pinCount)
  ...
  return (
    <g>
      ...
      >
        <rect x={0} y={0} width={w} height={IFACE_HEIGHT} rx={3}
          fill={isHarting ? '#1f2937' : '#475569'} stroke={isSelected ? '#f59e0b' : '#9ca3af'} strokeWidth={isSelected ? 2 : 1.2} />
        {isHarting && (
          <>
            <rect x={-3} y={IFACE_HEIGHT / 2 - 5} width={4} height={10} rx={1} fill="#6b7280" />
            <rect x={w - 1} y={IFACE_HEIGHT / 2 - 5} width={4} height={10} rx={1} fill="#6b7280" />
          </>
        )}
        {/* Pin-Kontakte + Nummern */}
        {pins.map((_, i) => {
          const px = IFACE_PAD + i * IFACE_PIN_SPACING + IFACE_PIN_SPACING / 2
          return (
            <g key={i}>
              <circle cx={px} cy={IFACE_HEIGHT / 2} r={isHarting ? 2 : 2.5} fill={isHarting ? '#fbbf24' : '#1f2937'} stroke="#111827" strokeWidth={0.5} />
              <text x={px} y={IFACE_HEIGHT - 4} textAnchor="middle" fontSize={5} fill="#94a3b8" fontFamily="monospace">{i + 1}</text>
            </g>
          )
        })}
        {/* Beschriftung */}
